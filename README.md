...
The pack auditor in [scripts/audit_futures_pack.py](scripts/audit_futures_pack.py) checks that futures packs' summary JSON/CSV, trades CSVs, event traces, manifests, and public-data assumption contracts agree on fills, per-fill economics, lifecycle counts, public queue-consumption totals, markouts, and artifact hashes.
