...
The input may be a finalized schema-v3 `capture_....manifest.json`, one finalized `.ndjson.zst` segment, or a legacy `.ndjson[.gz]` tape. The local audit pack is written under `outputs/real_data_runs/<label>/`; labels are immutable and an existing `pack` is never overwritten. The generator creates the pack with `_INCOMPLETE.json` first, copies event, fill, and markout audits through fsynced partials, and removes the sentinel only after its independent audit passes. A copy, simulation, or audit failure therefore leaves a visible incomplete pack and any forensic partials. The committed publication path writes only `docs/real_data_runs/<label>.md` and `docs/real_data_runs/<label>.json`; raw input, event traces, CSVs, and local packs are not copied into docs. New JSON reports use `lob_sim.real_data_report.v3` and state the input identity, market/risk metrics, audit memory contract, benchmark context, source state, target-window status, and a separate `lob_sim.real_data_evidence.v1` claim gate. The report reproduces the engine integrity/evidence objects and labels capture validity, subsecond markouts, modeled PnL, and strategy/profitability research independently. A clean execution gate does not make a single tape an economic or holdout claim. Historical v1 reports remain labeled as pre-streaming evidence. Keep local-only raw data local; publish only hashes and report-only artifacts.
