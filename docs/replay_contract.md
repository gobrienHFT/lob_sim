...
- `captureMeta`: declares schema version, receipt-clock policy, independent routes, and validity intersection for schema-v3 captures. Once schema version 3 is declared, every subsequent non-metadata record must carry `recvSeq`, `recvMonotonicNs`, and `route`; envelope identity fields remain non-empty strings, and receipt identity/`streamEpoch`/`syncEpoch` values must be exact non-negative JSON integers (booleans, strings, and fractional values are rejected rather than coerced). Receipt-monotonic regressions invalidate the clock dimension and halt execution state.
