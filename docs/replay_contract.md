...
It verifies that each pack's `summary.json`, `summary.csv`, `trades.csv`, `event_trace.csv`, and `manifest.json` agree on row counts, fill-source counts, order lifecycle counters, queue-ahead-at-arrival metrics, public queue-consumption totals, source-split markouts, public-data assumptions, and output artifact hashes.
