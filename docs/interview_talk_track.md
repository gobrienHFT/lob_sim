# Options Demo Talk Track

## 15-second overview

This is a transparent options market-making case study, not production infrastructure. It shows how a dealer quotes around Black-Scholes fair value, skews quotes for inventory, tracks toxic versus non-toxic flow through signed markout, hedges net delta in the underlying, and decomposes realized versus unrealized PnL.
...
### How does reservation pricing work here?
...
Signed markout compares the fill price with the model fair value at a fixed future horizon from the realized path. Positive is good for the dealer, negative means the trade aged badly and indicates adverse selection.

### Why hedge delta but warehouse gamma and vega?

Delta is the fastest and cheapest risk to reduce because it can be hedged directly in the underlying. Gamma and vega are left on so the demo keeps the inventory and warehousing trade-off visible instead of pretending the book is fully neutralized.
