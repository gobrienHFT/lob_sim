...
- Resting queue-ahead is modeled explicitly at each price level; it does not identify historical participant priority.
...
- Output summaries report observed public queue-consumption lots, overlap-netted lots, modeled queue-consumption candidates, synthetic queue lots consumed, and unmatched lots separately for the selected scenario.
...
- A public trade that arrives before the modeled cancel acknowledgement can still fill the old quote. For schema-v3 receipt-order ties, market observations are applied before same-time actions, so a same-time trade can fill before the cancel acknowledgement; legacy coarse-timestamp rows retain the explicit action-first compatibility sensitivity and apply the acknowledgement first.
- Latency and public-L2 execution sensitivity are tested as model assumptions, not performance or fill-truth claims: `experiments/sweep_futures_latency.py` reruns a committed fixture across order-arrival/cancel-ack grids and mutually exclusive `trade`/`depth` signals, publishing the deterministic reference in `docs/strategy_results/futures_latency_sweep_reference.md` with aggregate-only bounded retention.
...
- Seeded generated event-stream tests cover non-negative lot/count invariants, lifecycle sanity, deterministic summary/trace hashes, gap-skip behavior, self-trade prevention, and non-crossed strategy state after cancellation.
- `scripts/check_futures_determinism.py` reruns a replay fixture multiple times and compares canonical SHA-256 hashes of the metrics summary and event trace. The event-trace hash is produced incrementally by a bounded sink, so the checker does not retain a tape-sized trace in memory; use a streaming audit export when row-level inspection is required:
