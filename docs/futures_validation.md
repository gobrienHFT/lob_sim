...
- `scripts/audit_futures_pack.py --committed-futures` checks that committed pack summary JSON/CSV, trades, event traces, manifests, and public-data assumptions agree on fill counts, fill-source attribution, public queue-consumption totals, source-split markouts, lifecycle counters, and artifact hashes.
