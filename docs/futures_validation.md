...
- `scripts/audit_futures_pack.py` checks that committed pack summaries, trades, event traces, and manifests agree on fill counts, fill-source attribution, public queue-consumption totals, source-split markouts, lifecycle counters, and artifact hashes.
