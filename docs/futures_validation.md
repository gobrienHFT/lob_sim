...
- Output summaries report observed public queue-consumption lots, modeled lots, and overlap-netted lots separately for depth reductions and `aggTrade` prints.
...
- Simulation summaries include `public_consumption_summary` so overlap reconciliation is auditable even when no strategy order filled.
