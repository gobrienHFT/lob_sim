...
class BookInvariantError(RuntimeError):
    """Raised before an invalid market-by-price state becomes observable."""
    ...
    def _validate_state(bids: Dict[int, int], asks: Dict[int, int]) -> None:
        ...
        if bids and asks:
            ...
            if best_bid >= best_ask:
                raise BookInvariantError(f"crossed or locked book: best_bid_tick={best_bid}, best_ask_tick={best_ask}")

    def apply_depth_update(self, bids: list[tuple[int, int]], asks: list[tuple[int, int]]) -> list[LevelChange]:
        # Stage both sides and validate before committing.  A malformed batch
        # must not leave a half-applied book that subsequent fills could see.
