...
def _build_markdown(summary: dict[str, object], summary_file: Path) -> str:
    ...
    lines = [
        ...
        f"- `quote_fill_probability`: {_fmt_num(summary.get('quote_fill_probability', 0))}",
        f"- `fills_per_quote_request`: {_fmt_num(summary.get('fills_per_quote_request', 0))}",
        f"- `fills_per_arrived_order`: {_fmt_num(summary.get('fills_per_arrived_order', 0))}",
